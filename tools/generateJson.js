...
_.each(settings.big_picture, function(landscape, name) {
  _.each(landscape.elements, function(element) {
    const hasCompanyCategory = (function() {
      var result = false;
      tree.map(function(node) {
    ...
    if (element.category && !hasCompanyCategory) {
      ...
      there is a big_picture.${name}.elements entry with category: "${element.category}" but we do not have that category "${element.category}" in the landscape.yml!`);
