...
const takeScreenshot = async ({ client, width, height, scale, path }) => {
  const clip = { x: 0, y: 0, width, height, scale }
  const result = await client.send('Page.captureScreenshot', { format: 'png', clip })
...
async function main() {
  ...
  await Promise.mapSeries([previews, full_sizes], async function(series) {
    for (const pageInfo of series) {
      const { url, scale, fileName, pdfFileName } = pageInfo
      ...
      const path = resolve(projectPath, 'dist', 'images', fileName)
      await takeScreenshot({ client: page._client, width, height, scale, path })
      if (pdfFileName) {
